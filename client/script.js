...
// CODE HERE
const hoverColor = evt => {
    evt.target.classList.toggle(`hover`);
};

sayHelloButton.addEventListener(`mouseover`, hoverColor);
...
// CODE HERE
sayHelloButton.addEventListener(`mouseout`, hoverColor);
...
// CODE HERE
const queryFunc = () => {
    axios.get(`http://localhost:3000/query-test?q=dog & q=cat`)
        .then((res) => {
            console.log(res.data);
        })
        .catch((err) => {
